...
function create_kube_key () {
  ...
  KUBE_SECRET_NAME=$4
  KEY=$( kube_ceph_keyring_gen ${CEPH_KEYRING} ${CEPH_KEYRING_TEMPLATE} )
  kubectl get --namespace ${DEPLOYMENT_NAMESPACE} secrets ${KUBE_SECRET_NAME}
  if [[ $? > 0 ]] ; then
...
type: Opaque
data:
  ${CEPH_KEYRING_NAME}: ${KEY}
EOF
    cat manifest.yaml
    kubectl create --namespace ${DEPLOYMENT_NAMESPACE} -f manifest.yaml --validate=false
  fi
}
